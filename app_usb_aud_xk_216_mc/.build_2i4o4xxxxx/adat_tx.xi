# 1 "/Users/cwu/Desktop/Audio_Sound/module_adat_tx/src/adat_tx.xc"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 142 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/Users/cwu/Desktop/Audio_Sound/module_adat_tx/src/adat_tx.xc" 2
...
# 10 "/Users/cwu/Desktop/Audio_Sound/module_adat_tx/src/adat_tx.xc" 2
...
int clz(unsigned x);
# 11 "/Users/cwu/Desktop/Audio_Sound/module_adat_tx/src/adat_tx.xc" 2
...
# 12 "/Users/cwu/Desktop/Audio_Sound/module_adat_tx/src/adat_tx.xc" 2
# 1 "/Users/cwu/Desktop/Audio_Sound/module_adat_tx/src/adat_lookups.h" 1
...
# 13 "/Users/cwu/Desktop/Audio_Sound/module_adat_tx/src/adat_tx.xc" 2
...
# 46 "/Users/cwu/Desktop/Audio_Sound/module_adat_tx/src/adat_tx.xc"
    if (last_lookup & 0x80) {
    ...
    for (int i = 0; i < 8; i++) {
      if (i == 2 || i == 4 || i == 6) {
        ...
        w[i] = __builtin_in_uint(c_data);
        w[i + 1] = __builtin_in_uint(c_data);
      }
#pragma loop unroll(3)
 for (int j = 24; j >= 8; j -= 8) {
        if (last_lookup & 0x80) {
          __builtin_out_uint(c_port, ~lookup40w[(w[i] >> j) & 0xFF]);
          last_lookup = ~lookup40b[(w[i] >> j) & 0xFF];
          __builtin_out_uchar(c_port, last_lookup);
        }
        else {
          __builtin_out_uint(c_port, lookup40w[(w[i] >> j) & 0xFF]);
          last_lookup = lookup40b[(w[i] >> j) & 0xFF];
...
void adat_transmit_until_ct_2x(chanend c_data, chanend c_port, int smux)
{
  ...
  while (!__builtin_testct(c_data)) {
    ...
    w[0] = __builtin_in_uint(c_data);
    w[1] = __builtin_in_uint(c_data);
# 125 "/Users/cwu/Desktop/Audio_Sound/module_adat_tx/src/adat_tx.xc"
 ...
 w[i] = __builtin_in_uint(c_data);
      ...
      if (last_lookup & 0x80000)
        ...
        next_lookup = lookup20[(w[i] >> 16) & 0xFF];
      __builtin_out_uint(c_port, __builtin_byterev((next_lookup << 20) | (last_lookup & 0xFFFFF)));
      __builtin_out_uchar(c_port, next_lookup >> 12);
...
void adat_tx(chanend c_data, chanend c_port)
{
  while (1) {
    ...
    for (int i = 0; i < 8; i++) {
      __builtin_in_uint(c_data);
    }
    __builtin_out_uint(c_port, 0);
    __builtin_out_uint(c_port, 0);
    __builtin_out_uint(c_port, 0);
    __builtin_out_uint(c_port, 0);
