...
#pragma loop unroll
            ...
            for(int i = 0; i < I2S_CHANS_ADC; i+=I2S_CHANS_PER_FRAME)
            {
                ...
                unsigned sample;
                //add code for BU6 for a test.Comment it for a while.
                //sample <<= 1;
                asm volatile("in %0, res[%1]" : "=r"(sample)  : "r"(p_i2s_adc[index++]));
                //add code for BU6.Comment it for a while.
                //unsigned sample_reverse;
                //add code for BU6.
                //sample_reverse = bitrev(sample);
                //Comment it now,it is no useful.
                //sample_reverse&=~0x00800000;
...
#pragma loop unroll
            for(int i = 0; i < I2S_CHANS_ADC; i += I2S_CHANS_PER_FRAME)
            {
                ...
                asm volatile("in %0, res[%1]" : "=r"(sample)  : "r"(p_i2s_adc[index++]));
////////////////////////////////
                //add code for BU6.Comment it for while.
                //unsigned sample_reverse;
                //add code for BU6.Comment it for while.
                //sample_reverse = bitrev(sample);
                //Comment it now,it is no useful.
                ...
                ///////////////
                if(buffIndex)
                    samplesIn_1[((frameCount-1)&(I2S_CHANS_PER_FRAME-1))+i] = /*sample_reverse*/bitrev(sample); // channels 1, 3, 5.. on each line.
                else
                    samplesIn_0[((frameCount-1)&(I2S_CHANS_PER_FRAME-1))+i] = /*sample_reverse*/bitrev(sample); // channels 1, 3, 5.. on each line.
...
)
{
    ...
    AudioHwInit(c_config/*, chan_with_usb_audio_core*/);
...
//Comment it.It is no useful now.
//        else if((MCLK_96 % curSamFreq) == 0)
//        {
//            mClk = MCLK_96;
//        }
