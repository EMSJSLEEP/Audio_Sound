...
//port for run LED.
on tile[AUDIO_IO_TILE] : out port run_led = PORT_RUN_LED;
...
void user_periodical_process_thread_2(streaming chanend chan_with_audio_io_core)
{
    timer t;
    unsigned int time ;
    const unsigned int period = 100000 * 400; // 100000 timer ticks = 1ms, here is 1s
    ...
    while (1) {
        ...
        case t when timerafter ( time ) :> void :
            ...
            //Get cat9555 io newest value.
            //Comment it for a while for running led blinking thread which is running in an independent core.
...
//This funccion is for an experiment of port counter.
void run_led_blink(void)
{
    ...
    while (1) {
        ...
        count += 30000;
        run_led @ count <: 0xf; // timed output
        }

        run_led <: 0x0 /*@ count*/; // timestamped output
        for(int i = 0; i < 5000; i++) {
        count += 30000;
        run_led @ count <: 0x0; // timed output
        }
    }
}
...
#define c_EANativeTransport_ctrl null
    ...
    {
        {
            /*user periodical process thread core.*/
...
//user periodical process thread in a new core.
void user_periodical_process_thread_1(streaming chanend chan_with_usb_audio_core)
{
    timer t;
    unsigned int time ;
    unsigned int period = 100000 * 1000; // 100000 timer ticks = 1ms
    ...
    while (1) {
        ...
        case t when timerafter ( time ) :> void :
            ...
            //Get cat9555 io newest value and share with other core.
            //Comment it for a while for running led blinking thread which is running in a dependent core.
...
)
{
    ...
    //Add code for an experiment of port counter.Delete it after the experiment.
    //run_led_blink();
    send_cat9555_io_to_usb_audio_core(chan_with_usb_audio_core);
    par
    {
        /*user periodical process thread core.*/
        {
           //Do an extra experiment for led blinking thread which is running on a dependent core.
