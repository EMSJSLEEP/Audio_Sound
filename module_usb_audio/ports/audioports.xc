...
unsigned int divide, unsigned curSamFreq)
{
    ...
    if (divide == 1) /* e.g. 176.4KHz from 11.2896 */
    {
        ...
        //add code for data line sampling delay test.Comment it for a while.Do an experiment after an hour.
        //set_clock_rise_delay(clk_audio_bclk, 512);//20ns,40ns,60ns,80ns,100ns
    }
    else
    {
    ...
    for(int i = 0; i < numPortsAdc; i++)
    {
        ...
        //add code for BU6.Set sample delay.
    ...
    //add code for data line sampling delay test.Comment it for a while.Do an experiment after an hour.
    //set_clock_rise_delay(clk_audio_bclk, 40);//20ns,40ns,60ns,80ns,100ns

    /* Start clock blocks ticking */
    start_clock(clk_audio_bclk);
    //add code for data line sampling delay test.Comment it for a while.Do an experiment after an hour.
    //set_clock_rise_delay(clk_audio_bclk, 10);//20ns,40ns,60ns,80ns,100ns
